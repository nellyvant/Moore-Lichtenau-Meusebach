...
```{r}
#plot redo
#) p3$data → die plots muss ich neu berechnen und einzeln abspeichern, schauen wie mean-deckung berechnet wird (also ist das der mean innerhalb der plots, wo die art vorkommt oder der mean über alle plots zusammen??, zweiteres wäre mist, weil in manchen plots kommt es ja gar nicht vor, das würde ja alles verfälschen) → momentan ist nach deckung sortiert, will ich niht lieber die 10 häufisgten statt die 10 mit der höchsten mittleren deckung??

p3 <- MT_subset %>%
  group_by(Art) %>%
  summarise(
    mean_deckung = mean(Deckung,na.rm=T),
    n = sum(Deckung > 0)
  ) %>%
  arrange(desc(n)) %>%
  slice_head(n = 10) %>%
...
merged_data <- results %>%
  ...
  left_join(layer_per_plot, by =c("Plot" = "plot.ID"))
