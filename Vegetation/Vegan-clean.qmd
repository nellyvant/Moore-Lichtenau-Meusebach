...
```{r}
#alternativ nach Deckung ordnen: ggplot(aes(x = reorder(Art, mean_deckung), y = mean_deckung)) & arrange(desc(n))
...
p3 <- MT_subset %>%
  ...
  theme_minimal()
#Anzahl von holzigen Arten in der Krautschicht
#ersetze Artnamen mit Pinus_sylvestris, Betula oder Frangula_alnus
MN_subset %>%
  filter(
    Art == "Vaccinium_myrtillus",
    Deckung > 0
  ) %>%
  count(Schicht, name = "Anzahl")
