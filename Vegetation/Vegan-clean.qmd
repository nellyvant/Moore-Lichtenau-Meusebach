...
comm_moose_dca <- comm_moose[
  rownames(comm_moose) != "225",
  ,
  drop = FALSE
]
...
```{r}
data_MN <- merged_data$site == "Meu_nass" 
...
```{r}
#Transekt WO
data_MN_WO <- merged_data$site == "Meu_nass" & merged_data$Transekt == "WO"
comm_MN_WO <- comm_matrix[data_MN_WO, , drop = FALSE]
...
#Transekt NS
data_MN_NS <- merged_data$site == "Meu_nass" & merged_data$Transekt == "NS"
comm_MN_NS <- comm_matrix[data_MN_NS, , drop = FALSE]


comm_MN_NS <- comm_MN_NS[
  rowSums(comm_MN_NS) > 0,
  colSums(comm_MN_NS) > 0,
  drop = FALSE
]
...
```{r}
data_MT <- merged_data$site == "Meu_trock" 
comm_MT <- comm_matrix[data_MT, , drop = FALSE]
...
```{r}
#Transekt WO
data_MT_WO <- merged_data$site == "Meu_trock" & merged_data$Transekt == "WO"
comm_MT_WO <- comm_matrix[data_MT_WO, , drop = FALSE]


comm_MT_WO <- comm_MT_WO[
  rowSums(comm_MT_WO) > 0,
  colSums(comm_MT_WO) > 0,
  drop = FALSE
]
...
#Transekt NS
data_MT_NS <- merged_data$site == "Meu_trock" & merged_data$Transekt == "NS"
comm_MT_NS <- comm_matrix[data_MT_NS, , drop = FALSE]


comm_MT_NS <- comm_MT_NS[
  rowSums(comm_MT_NS) > 0,
  colSums(comm_MT_NS) > 0,
  drop = FALSE
]
...
```{r}
data_L <- merged_data$site == "Lic" 
comm_L <- comm_matrix[data_L, , drop = FALSE]
...
```{r}
#Transekt WO
data_L_WO <- merged_data$site == "Lic" & merged_data$Transekt == "WO"
comm_L_WO <- comm_matrix[data_L_WO, , drop = FALSE]
...
#Transekt NS
data_L_NS <- merged_data$site == "Lic" & merged_data$Transekt == "NS"
comm_L_NS <- comm_matrix[data_L_NS, , drop = FALSE]
...
DCAs nur für ein Transekt

```{r}
###Meusebch NAss Nord-> Süd

idx_MN <- merged_data$site == "Meu_trock" & merged_data$Transekt == "WO"
comm_MN <- comm_matrix[idx_MN, , drop = FALSE]
...
dca_df$site <- vegdata$site[match(dca_df$Name, vegdata$plot.ID)]
dca_df$Transektnummer <- vegdata$Transektnummer[match(dca_df$Name, vegdata$plot.ID)]
dca_df$Distanz <- dca_df$Transektnummer
