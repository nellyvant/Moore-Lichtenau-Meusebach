...
vorallem super wichtig wegen fehlender messwerte bei den lücken von 7 wochen\
daher ohne inerpolation:
...
```{r}
#Boxplots - Vergleich der Logger der einzelnen Flächen (äußerer und innerer Logger)
# Filter für Standort L und die gewünschten Logger
df_plot <- daily_logger_means %>%
  filter(Standort == "L", Logger %in% c("M", "N", "O", "S", "W")) %>%
  # In langes Format bringen: Sonne / Schatten
  pivot_longer(
...
# Boxplot erstellen, Sonne und Schatten nebeneinander pro Logger
ggplot(df_plot, aes(x = Logger, y = Temperatur, fill = Licht)) +
...
## 4.4 Temperaturen im Jahresverlauf

```{r}
#Mittelwerte für jede Fläche bilden
area_mean <- daily_logger_means %>%
  group_by(Standort, Datum) %>%
  summarise(
    Temp_sonne_area_mean = mean(Temp_sonne_mean, na.rm = TRUE),
    Temp_schatten_area_mean = mean(Temp_schatten_mean, na.rm = TRUE),
    .groups = "drop"
  )
summary(area_mean)

ggplot(area_mean, aes(x = Datum, 
                      y = Temp_sonne_area_mean, 
                      color = Standort)) +
  geom_line(linewidth = 1) +
  labs(
    title = "Tagesmitteltemperatur (Sonne) pro Standort",
    x = "Datum",
...
ich hab keine lust, den code für jedes parameter zu copy pasten

ich will eine tablle daily_logger_means machen, die mittelwerte von temp_sonne, temp_schatten, luftfeuchte, licht und taupukt direkt in einem hat\
aus dieser tabelle können wir dann wieder die extremwerttabelle machen und die plots\
ich probiere erstmal aus, ob das funktioniert. deswegen betrachte 5. mal als testfeld :)

überlegung: ich brauche ne stabdard deviation und vlt auch range (max-min). ist das sinnvoll, das für jeden tag für jeden logger zu machen? oder kann ich (zusätzlich) auch noch für jeden einzelnen logger herausfinden, was an diesem konkreten punkt die jahresmitteltemp, usw sind und dazu sd und range? ist das sinnvoll? denn die werte schwanken ja ganz klar im jahresverlauf... aber grade range und sd wären ja auch übers ganze jahr spannend\
ich werd wohl beides mal machen

## Tagesmittel

#ist quasi schon in daily_logger_summary enthalten

```{r}
#Berechnung der Tagesmittelwerte pro Logger in einer neuen Tabelle 
...
hier kommt noch dieses codechunk im original: #Minimum & Maximum der Temperaturmittelwerte für die einzelnen Logger
...
# 6. Analyse einzelner Parameter

## 6.1 Licht

!!!Licht sollte evtl log-transformiert werden!!

für die Weiterarbeit ist es vlt auch nice, die einzelnen parameter zu haben, da zb daily_logger_summary eine sehr riesige tabelle ist

dabei kann man je nach parameter noch weitere spannende infos rausfinden

```{r}
# Licht
#hier wäre es für die Mittelwerte mal spannend die Nacht (0 lux) auszuklammern und nur die Tagwerte anzuschauen, dazu die Sonnenscheindauer pro Tag bei jedem Logger
# um Mondlicht auszuschließen ist Sonnenaufgang/Untergang bei 1 lux als Grenzwert festgelegt
licht <- all_loggers %>%
  mutate(Datum = as.Date(DateTime)) %>%
  group_by(Logger_ID, Datum) %>%
  summarise(
    ...
    Sonnenaufgang = min(DateTime[Licht_lux > 1], na.rm = TRUE),
    Sonnenuntergang  = max(DateTime[Licht_lux > 1], na.rm = TRUE),
...
#sollte schon in summary pro logger drinne sein, außer der plot natürlich
