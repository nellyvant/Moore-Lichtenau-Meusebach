...
mtm_filled_long_Temp_sonne <- wide %>%
  select(DateTime, MTM_filled_rf) %>%
  rename(Temp_sonne_C = MTM_filled_rf) %>%
...
#RMSE berechnen aus den intern, durch das Modell berechneten MSE
rmse_oob <- sqrt(mod_rf$mse[length(mod_rf$mse)])
rmse_oob # = Werte weichen im Mitel ca.0,599 °C von echten Werten ab
...
nrmse_sd # 0,114 -> Modellfehler ist im Mittel ca. 11,4 % der Standardabweichung der MTM-Daten
```
...
#RMSE berechnen aus den intern, durch das Modell berechneten MSE
rmse_oob <- sqrt(mod_rf$mse[length(mod_rf$mse)])
rmse_oob # = Werte weichen im Mitel ca.3,142 % von echten Werten ab

#NRMSE berechnet auf der Basis der Spannweite der Daten
nrmse_oob <- rmse_oob / (max(wide$MTW, na.rm = TRUE) - min(wide$MTW, na.rm = TRUE))
nrmse_oob # = Größe des Fehlers im Verhältnis zur Variabilität der Daten - 0,046,  Fehler ist 4,6 % der gesamten Spannweite deiner Daten

#NRMSE berechnet auf der Basis der Standardabweichung der Daten
sd_mtw <- sd(wide$MTW, na.rm = TRUE)
nrmse_sd <- rmse_oob / sd_mtw
nrmse_sd # 0,195 -> Modellfehler ist im Mittel ca. 19,5 % der Standardabweichung der MTM-Daten
```
...
#RMSE berechnen aus den intern, durch das Modell berechneten MSE
rmse_oob <- sqrt(mod_rf$mse[length(mod_rf$mse)])
rmse_oob # = Werte weichen im Mitel ca.0,585 °C von echten Werten ab
...
sd_mtw <- sd(wide$MTW, na.rm = TRUE)
nrmse_sd <- rmse_oob / sd_mtw
nrmse_sd # 0,143 -> Modellfehler ist im Mittel ca. 14,3 % der Standardabweichung der MTM-Daten
```
...
#NRMSE berechnet auf der Basis der Spannweite der Daten
nrmse_oob <- rmse_oob / (max(wide$MNM, na.rm = TRUE) - min(wide$MNW, na.rm = TRUE))
nrmse_oob # = Größe des Fehlers im Verhältnis zur Variabilität der Daten - 0,019,  Fehler ist 1,9 % der gesamten Spannweite deiner Daten
