...
rmse_oob # = Werte weichen im Mitel ca. 1,29 °C von echten Werten ab
...
nrmse_sd # 0,121 -> Modellfehler ist im Mittel ca. 12,1 % der Standardabweichung der MTM-Daten
...
#RMSE berechnen aus den intern, durch das Modell berechneten MSE
rmse_oob <- sqrt(mod_rf$mse[length(mod_rf$mse)])
rmse_oob # = Werte weichen im Mitel ca.0,454 °C von echten Werten ab
...
sd_mtm <- sd(wide$MTM, na.rm = TRUE)
nrmse_sd <- rmse_oob / sd_mtm
nrmse_sd # 0,134 -> Modellfehler ist im Mittel ca. 13,4 % der Standardabweichung der MTM-Daten
...
#RMSE berechnen aus den intern, durch das Modell berechneten MSE
rmse_oob <- sqrt(mod_rf$mse[length(mod_rf$mse)])
rmse_oob # = Werte weichen im Mitel ca.1,8123 % von echten Werten ab
...
#RMSE berechnen aus den intern, durch das Modell berechneten MSE
rmse_oob <- sqrt(mod_rf$mse[length(mod_rf$mse)])
rmse_oob # = Werte weichen im Mitel ca.0,218 °C von echten Werten ab

#NRMSE berechnet auf der Basis der Spannweite der Daten
nrmse_oob <- rmse_oob / (max(wide$MTM, na.rm = TRUE) - min(wide$MTM, na.rm = TRUE))
nrmse_oob # = Größe des Fehlers im Verhältnis zur Variabilität der Daten - 0,007,  Fehler ist 0,7 % der gesamten Spannweite deiner Daten

#NRMSE berechnet auf der Basis der Standardabweichung der Daten
sd_mtm <- sd(wide$MTM, na.rm = TRUE)
nrmse_sd <- rmse_oob / sd_mtm
nrmse_sd # 0,039 -> Modellfehler ist im Mittel ca. 3,9 % der Standardabweichung der MTM-Daten
...
mod_rf <- randomForest(
  MNW ~ MNN + MNS + MNM + Hour_sin + Hour_cos + Weekday + DayOfYear + 
    DayOfYear_sin + DayOfYear_cos + is_weekend + diff_MNN_MNS + diff_MNN_MNM + diff_MNS_MNM,
  data = train,
  ntree = 500,        # je mehr, desto stabiler ist das Model
  mtry = 3,
  nodesize = 5,      # minimaler Blattknoten
  maxnodes = NULL,     # max. Blätter pro Baum
  importance = TRUE   
)
...
#RMSE berechnen aus den intern, durch das Modell berechneten MSE
rmse_oob <- sqrt(mod_rf$mse[length(mod_rf$mse)])
rmse_oob # = Werte weichen im Mitel ca.0,296 °C von echten Werten ab
...
sd_mnw <- sd(wide$MNW, na.rm = TRUE)
nrmse_sd <- rmse_oob / sd_mnw
nrmse_sd # 0,057 -> Modellfehler ist im Mittel ca. 5,7 % der Standardabweichung der MTM-Daten
...
#RMSE berechnen aus den intern, durch das Modell berechneten MSE
rmse_oob <- sqrt(mod_rf$mse[length(mod_rf$mse)])
rmse_oob # = Werte weichen im Mitel ca.0,347 °C von echten Werten ab
...
sd_mnw <- sd(wide$MNW, na.rm = TRUE)
nrmse_sd <- rmse_oob / sd_mnw
nrmse_sd # 0,084 -> Modellfehler ist im Mittel ca. 8,4 % der Standardabweichung der MTM-Daten
...
#RMSE berechnen aus den intern, durch das Modell berechneten MSE
rmse_oob <- sqrt(mod_rf$mse[length(mod_rf$mse)])
rmse_oob # = Werte weichen im Mitel ca.1,593 % von echten Werten ab
...
sd_mnw <- sd(wide$MNW, na.rm = TRUE)
nrmse_sd <- rmse_oob / sd_mnw
nrmse_sd # 0,104 -> Modellfehler ist im Mittel ca. 10,4 % der Standardabweichung der MTM-Daten
