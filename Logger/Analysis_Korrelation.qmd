...
```{r}
###### Meusebach trocken - Mitte Logger: fehlende Daten für inneren & äußeren Logger
...
# 3. Korrelationsmatrix der Logger-Spalten berechnen
# Nur numerische Spalten auswählen
cor_mat <- cor(
  wide %>% select(where(is.numeric)),
  use = "pairwise.complete.obs"
)

# 4. Korrelationsmatrix auf 2 Nachkommastellen runden
round(cor_mat, 2)

# 5. Korrelationswerte für Logger "MTM" sortiert
sort(cor_mat["MTM", ], decreasing = TRUE)
```

```{r}
# 1. Aus dem 'wide'-Datensatz werden alle Zeilen ausgewählt, bei denen 'MTM' nicht NA ist.
train <- wide %>%
  filter(!is.na(MTM))

# 2. Lineares Regressionsmodell erstellen
# 'MTM' wird als abhängige Variable genommen, 
# und 'MTN', 'MTS' und 'MTO' als unabhängige Variablen.
mod <- lm(MTM ~ MTN + MTS + MTO, data = train)

# 3. Zusammenfassung des Modells anzeigen
# relativ gute R² Werte, ca. 0.95
summary(mod)

# 4. Vorhersagen für alle Zeilen im Datensatz erstellen
pred <- predict(mod, newdata = wide)

# 5. Fehlende Werte in 'MTM' auffüllen
...
# 6. Validierung simulieren, zufällig 2000 bekannte MTM-Werte auswählen, um sie als "fehlend" zu markieren -> Vorhersagen -> messen, wie stark die Vorhersagen von den echten Werten abweichen
set.seed(42)  
...
### 4.1.1 Lineares Modell mit zeitlichen Features
...
wide$Hour <- as.numeric(format(wide$DateTime, "%H"))       # Stunde des Tages (0-23), 0 = 24.00/ 0 Uhr 
wide$Weekday <- as.factor(weekdays(wide$DateTime))         # Wochentag als Faktor
...
## 4.2 RandomForest (nicht linear, kann plötzliche Schwankungen besser erfassen)
...
mod_rf <- randomForest(
  ...
  data = train,
  ntree = 500,        # Anzahl Bäume
  mtry = 3,           # Anzahl Features, die jeder Baum zufällig auswählt (Standard ist sqrt(#Features))
  importance = TRUE   
)
...
## 4.2 MissForest (kann fehlende Messgrößen in mehreren Loggern erkennen und schätzen, aber ungenauere Vorhersage)

```{r}
library(missForest)

# 1️⃣ Nur numerische Logger + Zeitfeatures
df_mf <- wide %>%
  select(MTM, MTN, MTS, MTO, Hour, DayOfYear)

# 2️⃣ Weekday als Dummy-Spalten (Base R)
# weekdays() liefert Faktor, hier in 0/1-Matrix umwandeln
weekdays_factor <- as.factor(weekdays(wide$DateTime))
weekdays_dummy <- model.matrix(~ weekdays_factor - 1)  # -1 entfernt Intercept
colnames(weekdays_dummy) <- paste0("Weekday_", colnames(weekdays_dummy))

# 3️⃣ Alle numerischen + Dummy-Spalten zusammenfügen
df_mf <- cbind(df_mf, weekdays_dummy)

# 4️⃣ missForest ausführen
set.seed(42)
imputed <- missForest(df_mf, verbose = TRUE, maxiter = 10)

# 5️⃣ Aufgefüllter Datensatz
df_filled <- imputed$ximp
```
...
# -----------------------------
# 2️⃣ Nur numerische Logger + Zeitfeatures
# -----------------------------
df_mf <- wide %>%
  select(MTM, MTN, MTS, MTO, Hour, DayOfYear)

# Weekday als Dummy-Spalten (Base R)
weekdays_factor <- as.factor(weekdays(wide$DateTime))
weekdays_dummy <- model.matrix(~ weekdays_factor - 1)  # -1 entfernt Intercept
colnames(weekdays_dummy) <- paste0("Weekday_", colnames(weekdays_dummy))

# Alle numerischen + Dummy-Spalten zusammenfügen
df_mf <- cbind(df_mf, weekdays_dummy)

# -----------------------------
# 3️⃣ Testwerte markieren für RMSE
# -----------------------------
set.seed(42)
idx <- sample(which(!is.na(df_mf$MTM)), 2000)  # zufällige echte Werte
df_test <- df_mf
df_test$MTM[idx] <- NA  # diese Werte sollen imputiert werden

# -----------------------------
# 4️⃣ missForest ausführen
# -----------------------------
set.seed(42)
imputed <- missForest(df_test, verbose = TRUE, maxiter = 10)

# Aufgefüllter Datensatz
df_filled <- imputed$ximp

# -----------------------------
# 5️⃣ RMSE für Testwerte berechnen
# -----------------------------
rmse_mf <- sqrt(mean(
  (df_filled$MTM[idx] - df_mf$MTM[idx])^2
))
rmse_mf
```
...
# Daten mittels Model berechnen - Bsp. MTW
#install.packages("missForest")
library(missForest)

# MT-Daten auswählen
# Angenommen MTW enthält alle Logger-Messungen von MTW
MT_data <- MT_data %>%
  mutate(DateTime = as.POSIXct(DateTime, format="%Y-%m-%d %H:%M:%S"))
...
# Nur numerische Messwerte auswählen für MissForest
numeric_cols <- MT_data %>%
  select(-DateTime, -Standort, -Logger, -Logger_ID) %>%
  mutate(across(everything(), as.numeric))

# MissForest Imputation durchführen
set.seed(123)  # für Reproduzierbarkeit
imputed <- missForest(numeric_cols)

# Ergebnisse wieder zusammenführen
MTW_imputed <- cbind(meta, imputed$ximp)

```

Mittels Mice Messwerte schätzen/erzeugen
