...
```{r}
# welche der fehlenden Messwerte sind mit welchen vorhandenen am stärksten korreliert 
df <- all_loggers %>%
  filter(
    Logger_ID == "LM",
    Standort == "L"
...
**Meusebach trocken - Logger Mitte (Bsp. Temp)**
...
# Trainingsdatensatz (nur Zeilen mit MTM-Werten)
train <- wide %>%
  filter(!is.na(MTM))
...
# Modellübersicht: wichtige Features
mod_rf
importance(mod_rf)
...
rmse_rf # deutlich besser !!!! random Modell besser als linear 
```
